    ...
    extends QAgent
{
                                    ...
                                    final Mino potentialAction)
    {
        ...
        */
        ...
        for (int row = 0; row < flattenedImage.getShape().getNumRows(); row++) {
            ...
            if (lineCleared) {
                 // Award bonus for each line cleared
                clearedLinesBonus += 1.0;
            }
        }
        ...
        for (int col = 0; col < Board.NUM_COLS - 1; col++) {
            ...
            // Add the difference in heights between adjacent columns
    ...
    public Mino getExplorationMove(final GameView game)
    {
        ...
        for(int i = 0; i < numMinoPositions; i++){
            Matrix curr = this.getQFunctionInput(game, game.getFinalMinoPositions().get(i));
            // gets the Q function
            try {
                qFunctions[i] = Math.exp(this.initQFunction().forward(curr).get(0, 0));
            }
            catch (Exception e) {
                e.printStackTrace();
                System.exit(-1);
        ...
        // normalizing based on sum
        double[] potentialOutcomes = new double[numMinoPositions];
        for (int i = 0; i < numMinoPositions; i++) {
            potentialOutcomes[i] = qFunctions[i] / sum;
        }

        // find min val
        for (int i = 0; i < numMinoPositions; i++) {
            if (potentialOutcomes[i] < minVal) {
                minValPosition = i;
                minVal = potentialOutcomes[i];
            }
        }
        return game.getFinalMinoPositions().get(minValPosition);
    }
    ...
    public double getReward(final GameView game)
    {
        ...
        // calculate to encourage column height uniformity (reduce variance)
        double meanHeight = totalHeight / (double) Board.NUM_COLS;
        double heightVar = 0;
        for (int i = 0; i < columnHeights.length; i ++) {
            heightVar += Math.pow(columnHeights[i] - meanHeight, 2);
        }
        heightVar /= Board.NUM_COLS;
        ...
        reward -= heightPen * 20.0;
        reward -= heightVar * 50.0;
